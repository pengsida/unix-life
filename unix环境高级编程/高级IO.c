...
14.2 记录锁
{
    ...
    锁的隐含和释放
    {
        1.任何时候关闭一个描述符时，则该进程通过一描述符可以引用的锁都将被释放。
        例子
        {
            fd1 = open(pathname,...);
            read_lock(fd1,...);
            fd2 = dup(fd1);
            close(fd2);
            // 当close(fd2)后，fd1上设置的锁都将被释放。
        }

        2.由fork产生的子进程不继承父进程所设置的锁。只有当子进程调用fcntl函数才能获得它自己的锁。
        3.如果一个文件描述符设置了close-on-exec标志，当执行exec后，该进程将关闭文件描述符，而文件描述符对应的文件的所有锁都将被释放。
    }

    强制性锁：强迫内核对每一个open、read和write系统调用都进行检查，检查调用进程对正在访问的文件中是否违背了某一把锁的作用。
}