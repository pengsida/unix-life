...
4.5 保护
{
    保护机制：当程序对内存空间执行了一次非期望的引用，保护机制可以阻止这种操作并且报告此类事件。
    ...
    4.5.1 段级保护
    {
        ...
        4.5.1.1 段限长limit检查
        {
            段描述符的段限长字段用于防止程序或过程寻址到段外内存位置。

            段选择符可以是0，这样的选择符指向GDT表中第一个不用的描述符项。这个描述符可以被加载进一个段寄存器中，但是任何使用这种描述符引用内存都将产生一个一般保护性异常。
        }

        4.5.1.2 段类型TYPE检查
        {
            除了代码和数据段描述符，处理器还有系统段描述符和门描述符。
            系统段描述符和门描述符可以用于管理任务以及异常和中断。
            并非所有的描述符都定义了一个段，门描述符中存放着指向一个过程入口点的指针。
            段描述符中的S标志和类型字段TYPE用于对非法使用段或门导致的编程错误进行检测。

            处理器可以识别以下三种类型的特权级
            {
                1.当前任务特权级CPL。它存放在CS和SS段寄存器的位0和位1中。CPL等于当前代码段的特权级。
                2.DPL是一个段或门的特权级。
                3.请求特权级RPL是一种赋予段选择符的超越特权级，它存放在选择符的位0和位1。
            }
        }
    }
    ...
    4.5.2 访问数据段时的特权级检查
    {
        为了访问数据段中的操作数，数据段的段选择符必须被加载进数据段寄存器。
        在把一个数据段选择符加载进段寄存器之前，处理器会进行特权级检查。操作系统会比较任务的CPL、段选择符的RPL和段描述符的DPL。只有当段的DPL大等于CPL和RPL的时候，处理器才会把选择符加载进段寄存器。
        在把一个堆栈段选择符加载进段寄存器之前，处理器会检查CPL、RPL和DPL是否相同。只有三者都一样，才能加载段选择符。
    }

    4.5.3 代码段之间转移控制时的特权级检查
    {
        当将程序控制权从一个代码段转移到另一个代码段时，目标代码段的段选择符必须加载进代码段寄存器中。
        
        4.3.1 JMP、CALL和RET指令的实现方法
        {
            JMP、CALL指令通过以下四种方法来引用另外一个代码段
            {
                目标操作数含有目标代码段的段选择符
                目标操作数指向一个调用门描述符，而该描述符中含有目标代码段的选择符。
                目标操作数指向一个TSS，而该TSS中含有目标代码段的额选择符。
                目标操作数指向一个任务门，该任务门指向一个TSS，而TSS中含有目标代码段的选择符。

                目标操作数含有目标代码段的段选择符的情况
                {
                    当访问非一致代码段时，调用程序的CPL一定要等于目标代码段描述符的DPL。RPL一定要小等于CPL。
                    当访问一致性代码段时，调用者的CPL一定要大等于DPL，此时忽略了对RPL的检查。
                }
            }
        }
        ...
        4.3.2 门描述符
        {
            ...
            调用门描述符可以存放在GDT或LDT中，不能存放在IDT中。
            为了访问调用门，调用者CPL必须小等于调用门段选择符的RPL，也必须小等于调用门段描述符的DPL。

            我们为CALL或JMP指令的操作数提供了一个远指针，该指针中的段选择符用于指定调用门。
            调用门中有段选择符，用于指定要访问的代码段。
        }
    }
}